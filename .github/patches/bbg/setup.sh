...
setup_baseband_guard() {
    ...
    if [ -n "$ref" ]; then
        echo "[ERROR] Vendored CI setup does not fetch refs. Update .github/patches/bbg instead."
        exit 1
    fi

    if [ ! -d "$BBG_DIR" ]; then
        echo "[ERROR] Vendored Baseband-guard tree not found at $BBG_DIR."
        exit 1
    fi

    echo " - vendored repo ready"
